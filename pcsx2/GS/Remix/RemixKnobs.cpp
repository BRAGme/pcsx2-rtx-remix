...
namespace remix_ps2
{
	namespace
	{
		...
		const knob s_knobs[] = {
			...
			{"MATREBUILD", "Textures and Materials", "Material refresh interval", knob_type::Integer, 0, 0, 100000, 30,
				nullptr,
				"Frames between re-resolving a material's texture. 0 (default) disables it. Leave it "
				"off: a rebuild swaps the material handle without invalidating cached meshes, so "
				"with stable mesh identity on, meshes keep the old handle until it is destroyed and "
				"their surfaces go white on a timer. Diagnostic only.",
				false},
			...
			{"BEACON", "Diagnostics", "Empty-frame beacon", knob_type::Boolean, 0, 0, 1, 1, nullptr,
				"Draws a white triangle once the scene has submitted no geometry for about two seconds, to show the runtime is alive with nothing to render. Off by default: a load screen or a menu legitimately submits no geometry for far longer than that, and the triangle then covers a picture that is working. Turn it on when a black screen needs telling apart from a dead runtime.",
				false},
